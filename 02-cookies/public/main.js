...
function restoreTheme() {
  ...
  console.log('THEME 1: ', cookie);
  cookie = cookie.find(part => part.startsWith('themeName'));
  console.log('THEME 2: ', cookie);
  if (cookie) cookie = cookie.split('=')[1];
  console.log('THEME 3: ', cookie);
...
function storeName(displayName) {
  document.cookie = `displayName=${displayName};max-age=60`;
}

// For restoring user's display name from cookies, if set in the past
function restoreName() {
  let cookie = document.cookie.split('; ');
  console.log('NAME  1: ', cookie);
  cookie = cookie.find(part => part.startsWith('displayName'));
  console.log('NAME  2: ', cookie);
  if (cookie) cookie = cookie.split('=')[1];
  console.log('NAME  3: ', cookie);
